...
export function buildCapabilities(mentions: Mention[]): Capabilities {
  ...
  for (const m of mentions) {
    ...
    else if (m.kind === "mcp") tools.add(m.id.startsWith("mcp.") ? m.id : `mcp.${m.id}`);
    else if (m.kind === "skill") skills.add(m.id);
  }
  return { tools: [...tools], skills: [...skills] };
}

export const hasCapabilities = (c: Capabilities) => c.tools.length > 0 || c.skills.length > 0;
...
export type OrchestrateRequest = {
  /** Convenience copy of the latest operator turn as plain text. */
  message: string;
  /**
   * Full transcript in OpenAI chat format. A turn that carries attachments
   * ships `content` as an array of typed blocks (`text`, `image_url`,
   * `input_audio`, `file`) — never a flat string, or the model is blind to it.
   */
