...
def _list_models(client) -> list[str]:
    ...
    try:
        ...
        sys.stderr.write(f"[mlx_runner] models.list failed: {e}\n")
        _MODEL_CACHE = []
    return _MODEL_CACHE
...
def _resolve_model(client, requested: str) -> str:
    ...
    if sentinel:
        ...
        sys.stderr.write(f"[mlx_runner] model auto-resolved → {chosen} (candidates={loaded})\n")
        return chosen
    if req in loaded:
        return req
    chosen = path_like or loaded[0]
    sys.stderr.write(f"[mlx_runner] model coerced: requested={req} → chosen={chosen} (loaded={loaded})\n")
    return chosen
...
def _render_chat_prompt(messages: list[dict]) -> tuple[str, list[str]]:
    ...
    if fn is None:
        ...
        raise RuntimeError(
            f"chat template family '{family}' not implemented in mlx_runner.py "
            f"(known: {known}). Fix: pick a registered family in /system-engine → "
            f"Models editor, or add the renderer in mlx_runner.py + chat-templates.mjs."
        )
...
def stream_chat(payload: str, system_prompt: str | None = None,
    ...
    sys.stderr.write(
        f"[mlx_runner] sampling temp={sampling.get('temperature')} top_p={sampling.get('top_p')} "
        f"max_tokens={sampling.get('max_tokens')} stop={sampling.get('stop')} "
        f"extra={sampling.get('extra_body')}\n"
    )
    sys.stderr.write(
        f"[mlx_runner] env_diag family={(os.getenv('ELARA_LLM_TEMPLATE_FAMILY') or os.getenv('LLM_CHAT_TEMPLATE') or 'qwen2.5')} "
    ...
    if os.getenv("ELARA_DUMP_PROMPT"):
        try:
            ...
            sys.stderr.write("[mlx_runner] DUMP_PROMPT=" + json.dumps(dump, ensure_ascii=False) + "\n")
        except Exception as _e:
            sys.stderr.write(f"[mlx_runner] dump failed: {_e}\n")
    ...
    try:
        ...
        for chunk in stream:
            ...
            if ttft_ms is None:
                ...
                sys.stderr.write(f"[mlx_runner] ttft_ms={ttft_ms} model={mdl}\n")
            print(delta, end="", flush=True)
            total_chars += len(delta)
            guard_text += delta
            if _loop_watchdog_should_stop(guard_text):
                sys.stderr.write("[mlx_runner] loop watchdog stopped repeated stream\n")
                print("\n\n[Stopped: repeated output guard]", flush=True)
                break
        print("", flush=True)
        total_ms = int((time.time() - t0) * 1000)
        sys.stderr.write(
            f"[mlx_runner] done total_ms={total_ms} chars={total_chars} "
            f"finish_reason={finish_reason or '-'} max_tokens={sampling.get('max_tokens')}\n"
        )
    except Exception as e:
        sys.stderr.write(f"[mlx_runner] stream error ({type(e).__name__}): {e}\n")
        loaded = _list_models(client)
        ...
        if total_chars == 0 and is_404_like:
            ...
            if fallback and fallback != mdl:
                sys.stderr.write(f"[mlx_runner] retry with loaded model: {fallback} (candidates={loaded})\n")
                try:
                    stream = client.completions.create(
                        model=fallback,
                        prompt=prompt_text,
                    ...
                    for chunk in stream:
                        ...
                        guard_text += delta
                        if _loop_watchdog_should_stop(guard_text):
                            sys.stderr.write("[mlx_runner] loop watchdog stopped repeated fallback stream\n")
                            print("\n\n[Stopped: repeated output guard]", flush=True)
                            break
                    print("", flush=True)
                    retried = True
                    sys.stderr.write(f"[mlx_runner] fallback ok model={fallback} chars={total_chars}\n")
                except Exception as e2:
                    sys.stderr.write(f"[mlx_runner] fallback failed: {e2}\n")
