        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <Select
              ...
              options={["none", ...Array.from(new Set(vaultStore.items.map((s) => s.scope)))]}
              onChange={(vaultScope) => set({ vaultScope, vaultName: "", vaultField: "" })}
            />
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const matchedSecret = vaultStore.items.find((s) => s.scope === draft.vaultScope && s.name === newName);
                    set({ 
                      vaultName: newName, 
                      vaultField: matchedSecret?.kind || "api_key" 
                    });
                  }}
                  disabled={draft.vaultScope === "none"}
                  className={cn(selectCls, draft.vaultScope === "none" && "opacity-40")}
                >
                  <option value="" className="bg-panel">
                    — Select Secret —
                  </option>
                  {vaultStore.items
                    .filter((s) => s.scope === draft.vaultScope)
                    .map((s) => (
                      <option key={s.id} value={s.name} className="bg-panel">
                        {s.name} ({s.kind})
                      </option>
                    ))}
                </select>
                <ChevronDown
                  size={13}
                  className={cn("pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground/60", draft.vaultScope === "none" && "opacity-40")}
                />
              </div>
            </div>
            <div>
...
function WebhookCard({
  ...
  const tone = riskTones[webhook.risk as AdapterRisk] || "topaz";
  return (
    <motion.div
      layout
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.18, ease: [0.22, 1, 0.36, 1] }}
      className="glass flex flex-col rounded-xl border border-white/[0.06] p-4 transition-colors duration-150 hover:border-topaz/30"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-2.5">
          <span
            className="mt-[2px] flex size-8 shrink-0 items-center justify-center rounded-lg border border-topaz/30 bg-topaz/[0.08]"
            style={{ boxShadow: "0 0 18px -10px var(--topaz)" }}
          >
            <WebhookIcon size={15} strokeWidth={1.7} className="text-topaz" />
          </span>
          <div className="min-w-0">
            <div className="truncate text-[14px] text-foreground">{webhook.name || "untitled"}</div>
            <div className="truncate font-mono text-[11.5px] text-muted-foreground/60">
              {webhook.id}
            </div>
          </div>
        </div>
        <Toggle on={webhook.enabled} onClick={onToggle} />
      </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2.5">
        {(
          [
            ["category", webhook.category, "amethyst"],
            ["connection", webhook.connection, "sapphire"],
            ["runner", webhook.runner, "emerald"],
            ["risk", webhook.risk, tone],
          ] as const
        ).map(([lbl, value, t]) => (
          <div key={lbl}>
            <div className="mono-label">{lbl}</div>
            <div className="mt-1 font-mono text-[12.5px]" style={{ color: `var(--${t})` }}>
              {value}
            </div>
          </div>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        <Tag tone={webhook.vaultScope === "none" ? "platinum" : "topaz"}>
          vault ·{" "}
          {webhook.vaultScope === "none" ? "—" : `${webhook.vaultName}.${webhook.vaultField}`}
        </Tag>
        {webhook.requiresApproval && <Tag tone="ruby">approval</Tag>}
        {webhook.tags.map((t: string) => (
          <Tag key={t} tone="sapphire">
            {t}
          </Tag>
        ))}
      ...
      <div className="mt-4 flex items-center gap-2">
        <button
          onClick={onEdit}
          className="flex flex-1 items-center justify-center gap-2 rounded-lg border border-white/[0.07] bg-raised/30 py-2 font-mono text-[12px] text-foreground/90 transition-all duration-150 hover:border-topaz/40 hover:text-topaz"
        >
          <Pencil size={12} strokeWidth={1.8} /> Edit
        </button>
        <button
...
function WebhookDialog({
  ...
  const set = (p: Partial<Webhook>) => setDraft((d) => ({ ...d, ...p }));
  ...
  return (
    ...
    >
      <motion.div
        ...
        transition={{ duration: 0.16, ease: [0.22, 1, 0.36, 1] }}
        onClick={(e) => e.stopPropagation()}
        className="glass my-6 w-full max-w-[680px] rounded-xl border border-topaz/30 p-6 shadow-[0_0_80px_-40px_var(--topaz)]"
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-[17px] font-medium tracking-tight text-foreground">
              {webhook.name ? `Edit · ${webhook.name}` : "New Webhook"}
            </h3>
            <p className="mt-1.5 max-w-[480px] text-[12.5px] leading-relaxed text-muted-foreground/75">
              Configure an inbound webhook endpoint. Adapters dictionaries are shared.
            </p>
          </div>
          <button onClick={onClose} aria-label="Close" title="Close">
            <X
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <div className="mono-label mb-1.5">slug</div>
              <input
                value={draft.slug}
            ...
            <div>
              <div className="mono-label mb-1.5">url override (optional)</div>
              <input
                value={draft.urlOverride}
          ...
          <div className="flex flex-wrap items-end gap-6">
            ...
            <div className="flex items-center gap-2.5 pb-2">
              <Toggle on={draft.ingestToRag} onClick={() => set({ ingestToRag: !draft.ingestToRag })} />
              <span className="font-mono text-[12.5px] text-muted-foreground/80">ingest to rag</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 flex items-center justify-end gap-2">
          ...
          <button
            ...
            className="rounded-lg border border-topaz/50 bg-topaz/15 px-4 py-2 font-mono text-[12.5px] text-foreground shadow-[0_0_26px_-10px_var(--topaz)] transition-colors hover:bg-topaz/25"
          >
            Save
          </button>
        </div>
...
function DictionariesAccordion({ dictOpen, setDictOpen, dict }: { dictOpen: boolean, setDictOpen: React.Dispatch<React.SetStateAction<boolean>>, dict: any }) {
  return (
    <div className="glass mt-8 rounded-xl border border-white/[0.06] p-4">
      <button
...
function WebhooksTab() {
  const { webhooks, create, update, remove, toggle } = useWebhooks();
  const { dict, ctx: ownerCtx } = useAdapters();
  ...
  useEffect(() => {
    const handleNew = () => setEditing({ ...emptyWebhook, id: "", createdAt: Date.now() } as Webhook);
    window.addEventListener("sovereign:new-webhook", handleNew);
    return () => window.removeEventListener("sovereign:new-webhook", handleNew);
  }, []);
  ...
  return (
    <div className="space-y-6">
      <DictionariesAccordion dictOpen={dictOpen} setDictOpen={setDictOpen} dict={dict} />
        ...
        {["all", ...dict.category.map((d) => d.key)].map((c) => (
          <button
            ...
            className={cn(
              ...
              cat === c
                ? "border-topaz/45 bg-topaz/10 text-foreground"
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {visible.map((w) => (
          <WebhookCard
            ...
            onDelete={async () => {
              if (ownerCtx && !canEditOwned(w, ownerCtx)) {
                toast.error(editRefusal(w, ownerCtx));
                return;
              }
              const ok = await confirmAction({
                title: `Delete ${w.name || "untitled"}?`,
                body: "This webhook will be permanently removed from the registry.",
                confirmLabel: "Delete",
                tone: "ruby",
              });
              if (ok) remove(w.id);
        ...
        tab === "webhooks" ? (
          <div className="flex items-center gap-2">
            ...
            <MiniButton tone="emerald" onClick={() => {
              ...
              <Plus size={12} strokeWidth={2} /> New webhook
            </MiniButton>
          </div>
        ) : (
          <div className="flex items-center gap-2">
          ...
          {tab === "webhooks" ? (
            <>
              <p className="max-w-[760px] text-[13.5px] leading-relaxed text-muted-foreground/75">
                ...
                Delivery into RAG spaces is configured in the Knowledge Hub.
              </p>
              <div className="mt-8">
                <WebhooksTab />
              </div>
            ...
            <>
              <p className="max-w-[760px] text-[13.5px] leading-relaxed text-muted-foreground/75">
                Cloud · Network · Social · Content · AI · DB — one registry, flexible connection
                types. Every adapter binds a category, an auth connection and an execution runner,
                with optional vault-backed secrets.
              </p>
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {visible.map((a) => (
                  <AdapterCard
                    ...
                    onDelete={async () => {
                      if (!canEditOwned(a, ownerCtx)) {
                        toast.error(editRefusal(a, ownerCtx));
                        return;
                      }
                      const ok = await confirmAction({
                        title: `Delete ${a.name || "untitled"}?`,
                        body: "This adapter will be permanently removed from the registry.",
                        confirmLabel: "Delete",
                        tone: "ruby",
                      });
                      if (ok) removeAdapter(a.id);
                    }}
                  />
                ))}
              </div>
...
export const Route = createFileRoute("/adapters")({
  validateSearch: (search: Record<string, unknown>): { view: "adapters" | "webhooks" } => ({
    view: search["view"] === "webhooks" ? "webhooks" : "adapters",
  }),
  head: () => ({
    meta: [
