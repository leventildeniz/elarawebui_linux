...
export function OutputBindingCard({
  ...
  node: WorkflowNode;
  disabled?: boolean;
  onChange: (binding: OutputBinding, summary: string) => void;
}) {
  ...
  const set = (patch: Partial<OutputBinding>) => {
    const next = { ...b, ...patch };
    onChange(next, sinkSummary(next, webhooks.find((w) => w.id === next.webhookId)?.name));
  };
  ...
  return (
    <div className="rounded-xl border border-emerald/30 bg-emerald/[0.06] p-3">
      <div className="mb-2 flex items-center gap-2">
        {Icon && <Icon className="h-3.5 w-3.5 text-emerald" strokeWidth={1.7} />}
