...
const field =
  "w-full min-w-0 rounded-lg border border-border/70 bg-raised/40 px-2.5 py-1.5 font-mono text-[12px] text-foreground/90 outline-none transition-colors focus:border-sapphire/50";
...
export function TriggerScheduleCard({
  ...
  node: WorkflowNode;
  disabled?: boolean;
  onChange: (schedule: TriggerSchedule, summary: string, binding: TriggerBinding) => void;
}) {
  ...
  const hook = webhooks.find((w) => w.id === b.webhookId);
  const emit = (nextS: TriggerSchedule, nextB: TriggerBinding) => {
    const label = webhooks.find((w) => w.id === nextB.webhookId)?.name;
  ...
  return (
    <div className="rounded-xl border border-sapphire/30 bg-sapphire/[0.06] p-3">
      <div className="mb-2 flex items-center gap-2">
        {Icon && <Icon className="h-3.5 w-3.5 text-sapphire" strokeWidth={1.7} />}
