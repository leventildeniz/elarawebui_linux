...
import { useSkillSquads, useSkills } from "@/lib/skill-store";
import { confirmAction } from "./confirm-dialog";
import { cn } from "@/lib/utils";

/** Header squad bar for the Skills Engine — scopes the library to one squad. */
export function SkillSquadTabs() {
  const { skills, update } = useSkills();
  const { squads, active, setActive, addSquad, renameSquad, removeSquad } = useSkillSquads();
  ...
  const count = (squadName: string) => (skills || []).filter((s) => s.squad === squadName).length;
  ...
  const commitRename = (id: string, oldName: string, next: string) => {
    ...
    skills.filter((s) => s.squad === oldName).forEach((s) => update(s.id, { squad: clean }));
  };

  const deleteSquad = async (id: string, squadName: string, n: number) => {
    const ok = await confirmAction({
      title: "Delete this group?",
      body: `Delete squad "${squadName}"? Its ${n} skill${n > 1 ? "s" : ""} will move to Unassigned.`,
      confirmLabel: "Delete",
      tone: "ruby",
    });
    if (!ok) return;
    ...
    if (n > 0) {
      skills
        .filter((s) => s.squad === squadName)
        .forEach((s) => update(s.id, { squad: "Unassigned" }));
    }
    removeSquad(id);
