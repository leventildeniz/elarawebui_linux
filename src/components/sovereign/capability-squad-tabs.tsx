...
/** Header squad bar for the Capability Registry — scopes the library to one squad. */
export function CapabilitySquadTabs() {
  ...
  const count = (squadName: string) => (packs || []).filter((s) => s.squad === squadName).length;
  ...
  const commitRename = (id: string, oldName: string, next: string) => {
    ...
    packs.filter((s) => s.squad === oldName).forEach((s) => update(s.id, { squad: clean }));
  };

  const deleteSquad = async (id: string, squadName: string, n: number) => {
    const ok = await confirmAction({
      title: "Delete this group?",
      body: `Delete squad "${squadName}"? Its ${n} pack${n > 1 ? "s" : ""} will move to Unassigned.`,
      confirmLabel: "Delete",
      tone: "ruby",
    });
    if (!ok) return;
